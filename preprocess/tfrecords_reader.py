...
def save_actual_output(tfrecords_id, actual_output, write_path):
    if os.path.exists(write_path):
        ...
        with open(write_path, 'r') as json_file:
            ...
            write_data['actual_v'] = actual_output
        with open(write_path, 'w') as json_file:
            json.dump(write_data, json_file)
    else:
        with open(write_path, 'w') as json_file:
            output = { "id": tfrecords_id }
            output['actual_v']= actual_output
...
if __name__ == '__main__':
    ...
    for item in os.listdir(tfrecords_path):
        ...
        write_path = os.path.join('../data/actual_noslight', str(tfrecords_id) + '.json')

        if not os.path.exists(write_path) and match_tripids(tfrecords_id, tripids):
            #print(filenames)
            # Read tfrecords
            ...
            with tf.Session() as sess:
                ...
                # need to edit here
                save_actual_output(tfrecords_id,actions_list,write_path)
                sess.close()
